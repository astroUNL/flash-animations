	...
	public class PhaseDisc extends Sprite {
		...
		public function PhaseDisc(params:Object=null) {
			...
			if (params!=null) {
				...
				if (params.lightAlpha is Number) lightAlpha = params.lightAlpha;
				if (params.darkColor is Number) darkColor = params.darkColor;
				if (params.darkAlpha is Number) darkAlpha = params.darkAlpha;
		...
		public function update(...ignored):void {
			...
			_darkArea.graphics.beginFill(darkColor, darkAlpha);
			for (i=1; i<=n; i++) {
				angle = i*step;
			...
			_lightArea.graphics.beginFill(lightColor, lightAlpha);
			for (i=1; i<=n; i++) {
				angle = i*step;
				ax = -radius*Math.sin(angle);
				ay = -radius*Math.cos(angle);
