package {
	...
	public class ProtoSliderLogicCyclic {
		...
		function ProtoSliderLogicCyclic(initObject:Object) {
		...
		public function getValueObjectFromValue(x:Number):Object {
			...
			if (_cyclic) {
				var r:Number = _maxV - _minV;
				x = _minV + ((x - _minV)%r + r)%r;
			}
			else {
				if (x<_minV) x = _minV;
		...
		public function getIncrementedValueObject(valueObj:*, numTicks:int):Object {
			...
			if (_cyclic) {
				if (vObj.value<_minV || vObj.value>_maxV) vObj = getValueObjectFromValue(vObj.value);				
			}
			else {
