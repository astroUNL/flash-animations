	...
	public class StarField extends Sprite {
		...
		public function update(...argsIgnored):void {
			...
			for (i=0; i<_starsList.length; i++) {
				star = _starsList[i];
				if (star.isVariable) {
					sine = Math.sin((Math.PI/star.period)*(_epoch - star.phaseOffset));
					mag = star.mag + star.dip*sine*sine;
				}
				else {
					mag = star.mag;
				}
				f = _peakValue*Math.pow(10, (_saturationMagnitude-mag)/2.5);
				left = _starsList[i].x - _psfX;
				top = _starsList[i].y - _psfY;
		...
		public function set mappingMode(arg:String):void {
			if (arg!="linear" && arg!="gamma") return;
			_mappingMode = arg;
			generateLookupTable();
			generateNoisePixels();
			update();
		...
		public function set epoch(e:Number):void {
			_epoch = e;
		...
		public function setDimensions(w:Number, h:Number):void {
			...
			trace("numChunks: "+_numChunks);
			trace("chunkSize: "+_chunkSize);
		...
		private function clearBitmap():void {
			var startTimer:Number = getTimer();
			//_fieldBMD.fillRect(_fieldRect, 0xff0000);
			trace("clearBitmap: "+(getTimer()-startTimer));
		}
		...
		public function testAiry():void {
			...
			for (i=0; i<_psfRadius; i++) {
				...
				for (j=0; j<=i; j++) {
					...
					testArr[_psfRadius+i][_psfRadius-1-j] = a;
					testArr[_psfRadius+j][_psfRadius-1-i] = a;
					testArr[_psfRadius-1-j][_psfRadius-1-i] = a;
					testArr[_psfRadius-1-i][_psfRadius-1-j] = a;
					testArr[_psfRadius-1-i][_psfRadius+j] = a;
					testArr[_psfRadius-1-j][_psfRadius+i] = a;
					testArr[_psfRadius+j][_psfRadius+i] = a;
					testArr[_psfRadius+i][_psfRadius+j] = a;					
				}
			}
			trace("generatePSFData: "+(getTimer()-startTimer));
