...
package {
	...
	public class StarField extends Sprite {
		...
		public function StarField(initObj:* = undefined):void {
			...
			setNoiseParameters(10000, 10000);
			psfRadius = 7;			
			...
			if (_mappingMode=="linear") {
				for (var y:int = 0; y<h; y++) {
					for (var x:int = 0; x<w; x++) {
						...
						g = k*_fieldData[x+y*w];
						
						//g = 0xff*pow(log(_fieldData[x+y*w]), 0.5);
						
						//_testBA.writeUnsignedInt(0xff000000 | g | (g << 8) | (g << 16));
						
//						g = k*_fieldData[x][y];
						//g = k*_fieldData[x+y*w];
						//if (_invertMapping) g = 0xff - g;
			...
			//_testBA.position = 0;
			//_fieldBMD.setPixels(_testRect, _testBA);
		...
		private function generateNoiseData():void {
			...
			for (var i:int = 0; i<_width; i++) {
				...
				//	_noiseData.set(i, j, _noiseSigma*x1*f);
			//		_noiseData.set(i, j+1, _noiseSigma*x2*f);
		...
		private function generateFieldData():void {
			...
			var r:int = len - j;
			for (i=0; i<r; i++) {
				_fieldDataRaw[i] = _noiseData[j+i];
			}
			for (i=r; i<len; i++) {
				_fieldDataRaw[i] = _noiseData[i-r];
			}
			...
			*/
			...
			for (i=0; i<_starsList.length; i++) {
				...
				for (j=0; j<n; j++) {
					...
					//_fieldData[x+y*_width] += f*_psfData[j][k];
					_fieldDataRaw[x+y*_width] += f*_psfData[j][k];
						//_fieldData.set(x, top+k, _fieldData.get(x, top+k) + f*_psfData[j][k]);
					}
				}
			}
