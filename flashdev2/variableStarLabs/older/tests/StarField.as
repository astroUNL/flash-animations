...
package {
	...
	public class StarField extends Sprite {
		...
		// - noiseSource is a byte array of the noise values that were calculated when
		//   the noise profile was specified; it can be divided into numChunks segments
		...
		private var _noiseSource:ByteArray;
		...
		private var _fieldData:ByteArray;
		...
		public function StarField(initObj:* = undefined):void {
			...
			_noiseSource = new ByteArray();
			_noisePixels = new ByteArray();
			_fieldData = new ByteArray();
	...
	/*	
		...
		*/
		...
		public function setMappingMode(...args):void {
		...
		private function generateNoise():void {
			...
			for (i=0; i<n; i++) {
				...
				_noiseSource.writeDouble(_noiseMean + _noiseSigma*x1*f);
				_noiseSource.writeDouble(_noiseMean + _noiseSigma*x2*f);
			}	
